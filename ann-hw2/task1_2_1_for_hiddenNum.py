...
def cls_YorN(O, y):
    ...
    print(f'Accuracy: {sum(flag)/len(y):.7f}')
...
def main():
    ...
    for hh in range(1,15):
        parameters = build_nn(d_i=2, d_h=hh, d_o=3)
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, 15),  accu)
    plt.xlabel('d_h', fontsize=15)
    plt.ylabel('train accuracy', fontsize=15)
    plt.grid(linestyle='--')
    plt.tight_layout()
    plt.savefig('/data1/zjw/homework/ann_hw2/acc.png')
    plt.show()
