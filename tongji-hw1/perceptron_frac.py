...
def main():
    ...
    acc_t = []
    for f in np.arange(0.5, 0.9, 0.05):
        train_data = df.sample(frac=f, random_state=0, axis=0)
    ...
    plt.plot(np.arange(0.5, 0.9, 0.05), acc_t)
    plt.xlabel('frac', fontsize=15)
    plt.ylabel('test accuracy', fontsize=15)
    plt.grid(linestyle='--')
    plt.tight_layout()
    plt.savefig('/data1/zjw/homework/tongji_hw1/frac.png')
    plt.show()
