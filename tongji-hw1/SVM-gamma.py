...
def main():
    ...
    for g in np.arange(0.01, 1, 0.01):
        clf = svm.SVC(C=3, gamma=g, max_iter=200, kernel = 'rbf')
    ...
    plt.plot(np.arange(0.01, 1, 0.01), train_acc, label='train')
    plt.plot(np.arange(0.01, 1, 0.01), test_acc, label='test')
    plt.legend()
    plt.xlabel('gamma', fontsize=15)
    plt.ylabel('Accuracy', fontsize=15)
    plt.grid(linestyle='--')
    plt.tight_layout()
    plt.savefig('/data1/zjw/homework/tongji_hw1/svm-gamma.png')
    plt.show()
