...
def main():
    ...
    train_acc = []
    test_acc = []
    for c in np.arange(0.1, 5, 0.25):
        clf = svm.SVC(C=c, gamma=0.05, max_iter=200, kernel = 'rbf')
    ...
    plt.plot(np.arange(0, 5, 0.25), train_acc, label='train')
    plt.plot(np.arange(0, 5, 0.25), test_acc, label='test')
    plt.legend()
    plt.xlabel('C', fontsize=15)
    plt.ylabel('Accuracy', fontsize=15)
    plt.grid(linestyle='--')
    plt.tight_layout()
    plt.savefig('/data1/zjw/homework/tongji_hw1/svm-c.png')
    plt.show()
