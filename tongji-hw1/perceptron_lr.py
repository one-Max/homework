...
def main():
    ...
    for learning_rate in np.arange(0.1,2,0.1):
        ...
        for epoch in range(num_epochs):
            w, acc = train(X_e, label, w, learning_rate)
            acc_history.append(acc)

        plt.plot(acc_history, label=f'lr={learning_rate:.2f}')
    ...
    plt.ylabel('accu', fontsize=15)
    plt.grid(linestyle='--')
    plt.tight_layout()
    plt.savefig('/data1/zjw/homework/tongji_hw1/accu-curve.png')
    plt.show()

    plt.figure(figsize=(10, 6))
    plt.plot(np.arange(0.1,2,0.1), acc_test_history)
    plt.legend(loc="upper right")
    plt.xlabel('lr', fontsize=15)
    plt.ylabel('accu', fontsize=15)
    plt.grid(linestyle='--')
    plt.tight_layout()
    plt.savefig('/data1/zjw/homework/tongji_hw1/lr.png')
    plt.show()
