...
class Net_dropout(nn.Module):
    def __init__(self):
        super().__init__()
        # 32x32 -> 28x28    maxpool -> 14x14
        self.conv1 = nn.Conv2d(3, 6, 5)
        # 14x14 -> 10x10    maxpool -> 5x5
...
class Net_conv_kernelsize3(nn.Module):
    def __init__(self):
        super().__init__()
        # 32x32 -> 30x30    maxpool -> 15x15
        self.conv1 = nn.Conv2d(3, 6, 3)
        # 15x15 -> 13x13    maxpool -> 6x6
...
class Net_addconv(nn.Module):
    def __init__(self):
        super().__init__()
        # 32x32 -> 32x32
        self.conv0 = nn.Conv2d(3, 3, 3, stride=1, padding=1)
        # 32x32 -> 28x28    maxpool -> 14x14
        self.conv1 = nn.Conv2d(3, 6, 5)
        # 14x14 -> 10x10    maxpool -> 5x5
        self.conv2 = nn.Conv2d(6, 16, 5)
...
class Alexnet(nn.Module):
    def __init__(self, ):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=3, out_channels=128, kernel_size=7, stride=2, padding=2)
        ...
        self.L1 = nn.Linear(in_features=256*3*3, out_features=1024)
        self.L2 = nn.Linear(in_features=1024, out_features=512)
        self.L3 = nn.Linear(in_features=512, out_features=10)
        self.dropout = nn.Dropout(p=0.2)
...
def main(args):
    ...
    if args.eval:
        ...
        print("label    :", ' '.join('%s' % classes[labels[j]] for j in range(4)))
    ...
    for epoch in range(args.epochs):
        ...
        accdim.append(acc)
        lossdim.append(loss)
        print(f'Epoch:{epoch:3}, Loss:{loss:9.6f}, Accuracys:{acc:9.6f}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', default='/data1/zjw/homework/ann-hw5/cifar/model_storage/')
